...
import { MarkEntry, CurriculumStandard, Student, AnnualPlan, PlanWeek, ClassGroup } from '../types';
...
export default function MarkSheet() {
  ...
  const classStudents = useMemo(() => students.filter((s: Student) => s.classId === selectedClass), [students, selectedClass]);
  ...
  React.useEffect(() => {
    ...
    classStudents.forEach((student: Student) => {
        const found = marks.find((m: MarkEntry) => m.studentId === student.id && m.subject === selectedSubject && m.term === selectedTerm);
        if (found) {
  ...
  const handleSave = () => {
      // 1. Save Marks
      const newMarks = [...marks.filter((m: MarkEntry) => !(m.subject === selectedSubject && m.term === selectedTerm && classStudents.some((s: Student) => s.id === m.studentId)))];
  ...
  const injectRemediationSession = () => {
      const currentClassInfo = CLASSES.find((c: ClassGroup) => c.id === selectedClass);
      ...
      const remediationItem: CurriculumStandard = {
          id: remediationId,
          cycle: 'secondary', // Added missing property
      ...
      const targetPlan = annualPlans.find((p: AnnualPlan) => 
      ...
      if (targetPlan) {
          ...
          const weekIndex = targetPlan.weeks.findIndex((w: PlanWeek) => w.status !== 'COMPLETED');
          ...
          if (weekIndex !== -1) {
              ...
              const updatedPlans = annualPlans.map((p: AnnualPlan) => p.id === targetPlan.id ? { ...p, weeks: updatedWeeks } : p);
    ...
    <div className="pb-20 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 no-print">
            <select value={selectedClass} onChange={e => setSelectedClass(e.target.value)} className="p-3 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 dark:text-white">
                {CLASSES.filter((c: ClassGroup) => user.levels.includes(c.gradeLevel)).map((c: ClassGroup) => (
            ...
            <select value={selectedSubject} onChange={e => setSelectedSubject(e.target.value)} className="p-3 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 dark:text-white">
                {user.subjects.map((s: string) => <option key={s} value={s}>{s}</option>)}
            </select>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden print:border-black print:shadow-none">
            ...
            <div className="hidden print:block p-8 border-b-2 border-black text-center">
                ...
                <div className="flex justify-between mt-4 font-bold">
                    ...
                    <span>القسم: {CLASSES.find((c: ClassGroup) => c.id === selectedClass)?.name}</span>
            ...
            <table className="w-full text-sm text-left rtl:text-right">
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-slate-700 print:divide-black">
                    {classStudents.map((student: Student, idx: number) => {
